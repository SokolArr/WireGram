...
class DBSettings:
    ...
    async_engine = create_async_engine(
        url=settings.DB_WG_USER_URL_asyncpg, echo=True
    )
    session_factory = sessionmaker(sync_engine)
    async_session_factory = async_sessionmaker(
        async_engine, expire_on_commit=True
    )

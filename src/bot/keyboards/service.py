    ...
    kb = [
        [
            InlineKeyboardButton(
                text="✅ Создать конфиг",
                callback_data=f"serv_get_new_btn:{user_tg_id}:no_conf",
            )
        ],
        [
            InlineKeyboardButton(
...
def service_del_view(user_tg_id: int):
    kb = [
        [
            InlineKeyboardButton(
                text="⬅️ Назад", callback_data=f"menu_service_btn:{user_tg_id}"
            )
        ]
    ]
    return InlineKeyboardMarkup(inline_keyboard=kb)
...
def new_order_view(user_tg_id: int, config_name: str):
    kb = [
        [
            InlineKeyboardButton(
                text="💵 Оплатил!",
                callback_data=f"serv_payed_btn:{user_tg_id}:{config_name}",
            )
        ],
        [
            InlineKeyboardButton(
...
def new_conf_view(user_tg_id: int, config_name: str):
    kb = [
        [
            InlineKeyboardButton(
                text=f"{config_name}",
                callback_data=f"serv_chosse_btn:{user_tg_id}:{config_name}",
            )
        ],
        [
            InlineKeyboardButton(
                text="⬅️ Назад",
                callback_data=f"menu_service_btn:{user_tg_id}:{config_name}",
            )
        ],
    ]
    return InlineKeyboardMarkup(inline_keyboard=kb)
...
def actions_conf_kb(
    user_tg_id: int,
    config_name: str,
    is_renew_req: bool = False,
    is_pay_req: bool = False,
):
    kb = []
    if is_renew_req:
        kb.append(
            [
                InlineKeyboardButton(
                    ...
                    callback_data=f"serv_renew_btn:{user_tg_id}:{config_name}",
                )
            ]
        )
    if is_pay_req:
    ...
    kb.append(
        [
            InlineKeyboardButton(
                ...
                callback_data=f"serv_get_path_btn:{user_tg_id}:{config_name}",
            )
        ]
    )
    # kb.append([InlineKeyboardButton(text="⛔️ Удалить конфиг",
    # callback_data=f'serv_del_conf_btn:{user_tg_id}:{config_name}')])
    kb.append(
        [
            InlineKeyboardButton(
                text="⬅️ Назад",
                callback_data=f"menu_service_btn:{user_tg_id}:no_conf",
            )
        ]
    )
    return InlineKeyboardMarkup(inline_keyboard=kb)

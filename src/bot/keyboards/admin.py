...
def admin_menu_kb(user_tg_id: int):
    kb = [
        ...
        [InlineKeyboardButton(text="💵 Все заявки на оплату", callback_data=f'admin_all_conf_pay_btn:{user_tg_id}')]
    ]
    return InlineKeyboardMarkup(inline_keyboard=kb)

def access_request_kb(user_tg_id: int):
    kb = [
        [InlineKeyboardButton(text="✅ Принять", callback_data=f'admin_ar_acpt_btn:{user_tg_id}')],
        [InlineKeyboardButton(text="⛔️ Отклонить", callback_data=f'admin_ar_decl_btn:{user_tg_id}')]
    ]
    return InlineKeyboardMarkup(inline_keyboard=kb)
...
def conf_pay_request_kb(user_tg_id: int, config_name: str):
    kb = [
        [InlineKeyboardButton(text="✅ Принять оплату", callback_data=f'admin_conf_pay_acpt_btn:{user_tg_id}:{config_name}')],
        [InlineKeyboardButton(text="⛔️ Отклонить оплату", callback_data=f'admin_conf_pay_decl_btn:{user_tg_id}:{config_name}')]
    ]
    return InlineKeyboardMarkup(inline_keyboard=kb)
...
def conf_pay_requests_btn(orders: tuple):
    ...
    for order in orders:
        ...
        builder.row(InlineKeyboardButton(text=f"✅ {user_tg_id} {config_name}", callback_data=(f'admin_conf_pay_acpt_btn:{user_tg_id}:{config_name}')),
